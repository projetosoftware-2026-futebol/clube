...
AUTH_HEADERS = {"X-User-Id": "auth0|user-a"}
OTHER_AUTH_HEADERS = {"X-User-Id": "auth0|user-b"}
...
def test_create_and_get_clube_and_delete_clube(client):
    ...
    delete_response = client.delete(f"/clube/delete/{club_id}", headers=AUTH_HEADERS)
...
def test_create_two_clubes_and_list_and_delete_both(client):
    ...
    assert client.delete(f"/clube/delete/{club1['id']}", headers=AUTH_HEADERS).status_code == 200
    assert client.delete(f"/clube/delete/{club2['id']}", headers=AUTH_HEADERS).status_code == 200
...
def test_create_clube_duplicate_name(client):
    client.post("/clube/create", json={"name": "FC Dup", "league": "Ligue 1"}, headers=AUTH_HEADERS)

    response = client.post("/clube/create", json={"name": "FC Dup", "league": "Ligue 1"}, headers=AUTH_HEADERS)
...
def test_create_and_update_clube(client):
    ...
    update_response = client.put(
        f"/clube/update/{club_id}",
        json={"budget": 99_000_000},
        headers=AUTH_HEADERS,
    )
...
def test_create_clube_requires_logged_user(client):
    response = client.post("/clube/create", json={
        "name": "FC Sem Login",
        "league": "Premier League",
    })
...
def test_other_user_cannot_edit_clube(client):
    create_response = client.post("/clube/create", json={
        "name": "FC Owner",
        "league": "Premier League",
        "budget": 10_000_000,
    ...
    owner_view = client.get("/clube/get", headers=AUTH_HEADERS).get_json()
    other_view = client.get("/clube/get", headers=OTHER_AUTH_HEADERS).get_json()
    assert owner_view[0]["is_owner"] is True
    assert other_view[0]["is_owner"] is False

    update_response = client.put(
        f"/clube/update/{club_id}",
        json={"budget": 20_000_000},
        headers=OTHER_AUTH_HEADERS,
    )
    delete_response = client.delete(f"/clube/delete/{club_id}", headers=OTHER_AUTH_HEADERS)
    buy_response = client.post(
        "/clube/buy",
        json={"club_id": club_id, "player_id": 10},
        headers=OTHER_AUTH_HEADERS,
    )
    ...
    get_response = client.get(f"/clube/get/{club_id}", headers=AUTH_HEADERS)
    assert get_response.get_json()["budget"] == 10_000_000
...
def test_buy_and_sell_player(client):
    ...
    with patch("main.requests.get", return_value=_mock_player(available=True, value=1_000_000)), \
         patch("main.requests.patch"):
        client.post("/clube/buy", json={"club_id": club_id, "player_id": 10}, headers=AUTH_HEADERS)
...
def test_sell_player_not_in_clube(client):
    ...
    response = client.post(
        "/clube/sell",
        json={"club_id": club_id, "player_id": 99},
        headers=AUTH_HEADERS,
    )
    assert response.status_code == 404
